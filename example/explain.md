...
扩展只写入自己的配置项，不会清空你原来的其他设置。首次激活时，若工作区还没有快捷键/显示项，会补上默认值。搜索配置在你改过搜索选项后才会写入。

| 键 | 含义 |
|---|---|
| `tabGroups.shortcuts` | 可自定义命令快捷键；保存后同步到本机 `keybindings.json` |
| `tabGroups.display` | 显示相关：标记跳转提示、是否显示来源分支 |
| `tabGroups.search` | 侧边栏节点搜索：匹配方式、包含/排除文件夹 |
...
| 键 | 含义 | 默认 |
|---|---|---|
| `mode` | `fuzzy` 模糊（字符按顺序出现）/ `exact` 精准（包含完整查询串） | `fuzzy` |
| `include` | 搜索时包含的文件夹，逗号分隔；留空不限制 | `""` |
| `exclude` | 搜索时排除的文件夹，逗号分隔 | `""` |

这三项都可在侧边栏标题栏的「设置」或搜索框里改：显示页改完即存；快捷键页需点保存；搜索模式与文件夹在搜索条上改完即存。
